...
const TrafficLightsCam8 = () => {
    const [count, setCount] = useState(() => Number(localStorage.getItem("count8")) || 0)

    const handleTrafficLights = async (id) => {
        setCount(id)
        try {
            const response = await fetch(`http://localhost:5000/traffic_lights/cam8/${id}`);
            if (!response.ok) {
...
export default TrafficLightsCam8