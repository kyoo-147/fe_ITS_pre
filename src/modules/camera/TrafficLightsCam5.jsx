...
const TrafficLightsCam5 = () => {
    const [count, setCount] = useState(() => Number(localStorage.getItem("count5")) || 0)
    
    const handleTrafficLights = async (id) => {
        setCount(id)
        try {
            const response = await fetch(`http://localhost:5000/api_traffic_light_1/cam5/${id}`);
            if (!response.ok) {
...
export default TrafficLightsCam5