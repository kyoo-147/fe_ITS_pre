...
const TrafficLightsCam6 = () => {
    const [count, setCount] = useState(() => Number(localStorage.getItem("count6")) || 0)

    const handleTrafficLights = async (id) => {
        setCount(id)
        try {
            const response = await fetch(`http://localhost:5000/traffic_lights/cam6/${id}`);
            if (!response.ok) {
...
export default TrafficLightsCam6